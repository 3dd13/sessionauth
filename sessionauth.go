...
// user type.
func (sAuth SessionAuth) SessionUser(newUser func() User) martini.Handler {
	return func(s sessions.Session, c martini.Context, l *log.Logger) {
		userId := s.Get(sAuth.SessionKey)
		user := newUser()
...
func (sAuth SessionAuth) AuthenticateSession(s sessions.Session, user User) error {
	user.Login()
	return sAuth.UpdateUser(s, user)
}

// Logout will clear out the session and call the Logout() user function.
func (sAuth SessionAuth) Logout(s sessions.Session, user User) {
	user.Logout()
	s.Delete(sAuth.SessionKey)
}
